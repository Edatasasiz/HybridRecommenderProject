...
movie = pd.read_csv('Recommandation_Systems/datasets/movie_lens_dataset/movie.csv')
rating = pd.read_csv('Recommandation_Systems/datasets/movie_lens_dataset/rating.csv')
...
def create_user_movie_df():
    ...
    df = movie.merge(rating, how="left", on="movieId")
    comment_counts = pd.DataFrame(df["title"].value_counts())
    rare_movies = comment_counts[comment_counts["title"] <= 1000].index    # uzun sürüyorsa 10000 yap.
    common_movies = df[~df["title"].isin(rare_movies)]
    user_movie_df = common_movies.pivot_table(index=["userId"], columns=["title"], values="rating")
...
# Adım 1: user_same_movies listesi içerisindeki seçili user ile benzerlik gösteren kullanıcıların id’lerinin
...
# Okunma problemi var önce dataframe'e çevirelim.
corr_df = pd.DataFrame(corr_df, columns=["corr"])
# İndexlerde iki isim olduğu için onu adlandıralım.
corr_df.index.names = ['user_id_1', 'user_id_2']
# index isimlerini değişkene çevirelim.
...
# Zaten userid2 olanlar diğerleri olduğu için userid olarak adlandırabiliriz.
...
# Korelasyon 1 gelen zaten Sinan'dır onu çıkaralım. Olmayanları getirelim.
...
# Adım 2: Film id’sive her bir filme ait tüm kullanıcıların weighted rating’lerinin ortalama değerini içeren
# recommendation_df adında yeni bir dataframe oluşturunuz.

recommendation_df = top_users_ratings.groupby('movieId').agg({"weighted_rating": "mean"})
# movieidleri indexte değil sütunlarda istiyoruz. Reset index yapalım.
